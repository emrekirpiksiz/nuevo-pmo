...
import { Badge, Button, Card, Descriptions, Space, Tabs, Tag, Typography } from "antd";
    ...
    <Space direction="vertical" size="large" style={{ width: "100%" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start" }}>
        <div>
          ...
          <Typography.Text type="secondary">{project?.code} · {project?.customerName}</Typography.Text>
        </div>
        {project && (
          <Tag color={project.status === "Active" ? "green" : "blue"}>{project.status}</Tag>
        )}
      </div>

      <Card>
        <Descriptions column={2}>
          <Descriptions.Item label="Müşteri">{project?.customerName}</Descriptions.Item>
          <Descriptions.Item label="Kod">{project?.code}</Descriptions.Item>
          <Descriptions.Item label="Durum">{project?.status}</Descriptions.Item>
          <Descriptions.Item label="Oluşturma">{project && new Date(project.createdAt).toLocaleString()}</Descriptions.Item>
        ...
        items={[
          {
            key: "documents",
            label: "Dokümanlar",
            children: <DocumentsPanel projectId={id} mode="admin" />,
          },
          {
            key: "members",
            label: "Üyeler",
            children: <ProjectMembersPanel projectId={id} />,
          },
          {
            key: "plan",
            label: (
